...
namespace ZakYip.Sorting.RuleEngine.Infrastructure.Services
{
    ...
    public class WindowsFirewallManager
    {
        ...
        /// <summary>
        /// 异步检查并配置Windows防火墙，确保所需端口已开放
        /// Asynchronously check and configure Windows Firewall to ensure required ports are open
        /// </summary>
        /// <param name="ports">需要开放的端口列表</param>
        /// <returns>是否成功配置</returns>
        public async Task<bool> EnsureFirewallConfiguredAsync(IEnumerable<int> ports)
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                _logger.LogInformation("非Windows平台，跳过防火墙配置 | Not a Windows platform, skipping firewall configuration");
            ...
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                _logger.LogInformation("开始检查Windows防火墙配置 | Starting Windows Firewall configuration check");
                ...
                // 检查防火墙状态
                var firewallEnabled = await IsFirewallEnabledAsync();
                if (firewallEnabled)
                {
                    _logger.LogWarning("Windows防火墙已启用 | Windows Firewall is enabled");
                    
                    // 尝试关闭防火墙
                    var disabled = await DisableFirewallAsync();
                    if (disabled)
                    {
                        _logger.LogInformation("已成功关闭Windows防火墙 | Successfully disabled Windows Firewall");
                    }
                ...
                foreach (var port in ports)
                {
                    await EnsurePortRuleExistsAsync(port);
                }

                _logger.LogInformation("防火墙配置检查完成 | Firewall configuration check completed");
                return true;
            }, "异步配置Windows防火墙 | Async configure Windows Firewall", false);
        }

        /// <summary>
        /// 异步检查Windows防火墙是否启用
        /// Asynchronously check if Windows Firewall is enabled
        /// </summary>
        private async Task<bool> IsFirewallEnabledAsync()
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                var result = await ExecuteNetshCommandAsync("advfirewall show allprofiles state");
                // 检查输出中是否包含 "ON"
                return result.Contains("ON", StringComparison.OrdinalIgnoreCase);
            }, "检查防火墙状态 | Check firewall status", false);
        }
        ...
        /// <summary>
        /// 异步关闭Windows防火墙
        /// Asynchronously disable Windows Firewall
        /// </summary>
        private async Task<bool> DisableFirewallAsync()
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                _logger.LogInformation("尝试关闭Windows防火墙所有配置文件 | Attempting to disable Windows Firewall for all profiles");
                ...
                // 关闭所有配置文件的防火墙（域、专用、公用）
                var result = await ExecuteNetshCommandAsync("advfirewall set allprofiles state off");
        ...
        /// <summary>
        /// 异步确保指定端口的防火墙规则存在
        /// Asynchronously ensure firewall rule exists for the specified port
        /// </summary>
        private async Task EnsurePortRuleExistsAsync(int port)
        {
            await _safetyIsolator.ExecuteAsync(async () =>
            {
                var ruleName = $"ZakYip.Sorting.RuleEngine.Port{port}";
                
                // 检查规则是否已存在
                if (await CheckRuleExistsAsync(ruleName))
                {
                    _logger.LogInformation("端口 {Port} 的防火墙规则已存在 | Firewall rule for port {Port} already exists", port);
                    return;
                }
                ...
                // 添加入站规则
                var inboundResult = await ExecuteNetshCommandAsync(
                    $"advfirewall firewall add rule name=\"{ruleName}_Inbound\" " +
                    $"dir=in action=allow protocol=TCP localport={port}");
                ...
                // 添加出站规则
                var outboundResult = await ExecuteNetshCommandAsync(
                    $"advfirewall firewall add rule name=\"{ruleName}_Outbound\" " +
                    $"dir=out action=allow protocol=TCP localport={port}");
        ...
        /// <summary>
        /// 异步检查防火墙规则是否存在
        /// Asynchronously check if firewall rule exists
        /// </summary>
        private async Task<bool> CheckRuleExistsAsync(string ruleName)
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
        ...
        /// <summary>
        /// 异步执行netsh命令
        /// Asynchronously execute netsh command
        /// </summary>
        private async Task<string> ExecuteNetshCommandAsync(string arguments)
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                var processStartInfo = new ProcessStartInfo
        ...
        /// <summary>
        /// 异步配置所有网卡启用巨帧并设置传输缓存到最大值
        /// Asynchronously configure all network adapters to enable Jumbo Frames and set transmit buffers to maximum
        /// </summary>
        public async Task<bool> ConfigureNetworkAdaptersAsync()
        {
            if (!RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                _logger.LogInformation("非Windows平台，跳过网卡配置 | Not a Windows platform, skipping network adapter configuration");
            ...
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                ...
                // 获取所有物理网络适配器
                var adapters = await GetPhysicalNetworkAdaptersAsync();
                if (adapters.Count == 0)
                {
                    _logger.LogWarning("未找到物理网络适配器 | No physical network adapters found");
                    return false;
                ...
                foreach (var adapter in adapters)
                {
                    ...
                    // 启用巨帧 (Jumbo Frames)
                    if (await EnableJumboFramesAsync(adapter))
                    {
                        _logger.LogInformation("成功为适配器 {Adapter} 启用巨帧 | Successfully enabled Jumbo Frames for adapter {Adapter}", adapter);
                    }
                    
                    // 设置传输缓存到最大值
                    if (await SetMaxTransmitBuffersAsync(adapter))
                    {
                        _logger.LogInformation("成功为适配器 {Adapter} 设置最大传输缓存 | Successfully set maximum transmit buffers for adapter {Adapter}", adapter);
                    }
                    
                    // 关闭网卡节能功能
                    if (await DisablePowerSavingAsync(adapter))
                    {
                        _logger.LogInformation("成功为适配器 {Adapter} 关闭节能功能 | Successfully disabled power saving for adapter {Adapter}", adapter);
                        successCount++;
                    }
                ...
                _logger.LogInformation("网络适配器配置完成，成功配置 {Success}/{Total} 个适配器 | Network adapter configuration completed, successfully configured {Success}/{Total} adapters", successCount, adapters.Count);
                return successCount > 0;
            }, "异步配置网络适配器 | Async configure network adapters", false);
        }

        /// <summary>
        /// 异步获取所有物理网络适配器的名称
        /// Asynchronously get names of all physical network adapters
        /// </summary>
        private async Task<List<string>> GetPhysicalNetworkAdaptersAsync()
        {
            var adapters = new List<string>();
            await _safetyIsolator.ExecuteSilentAsync(async () =>
            {
                // 使用PowerShell获取物理网络适配器
        ...
        /// <summary>
        /// 异步为网络适配器启用巨帧
        /// Asynchronously enable Jumbo Frames for network adapter
        /// </summary>
        private async Task<bool> EnableJumboFramesAsync(string adapterName)
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                // 巨帧的典型值是9014字节（9KB）
                ...
                return success;
            }, $"为适配器 {adapterName} 启用巨帧 | Enable Jumbo Frames for adapter {adapterName}", false);
        }

        /// <summary>
        /// 异步设置网络适配器传输缓存到最大值
        /// Asynchronously set network adapter transmit buffers to maximum
        /// </summary>
        private async Task<bool> SetMaxTransmitBuffersAsync(string adapterName)
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                // 获取当前适配器的高级属性
                var getCommand = $"Get-NetAdapterAdvancedProperty -Name '{adapterName}' | Where-Object {{$_.DisplayName -like '*Transmit*' -or $_.DisplayName -like '*Send*' -or $_.DisplayName -like '*Buffer*'}}";
                await ExecutePowerShellCommandAsync(getCommand);
                ...
                foreach (var property in bufferProperties)
                {
                    ...
                    foreach (var maxValue in maxValues)
                    {
                        var result = await _safetyIsolator.ExecuteSilentAsync(async () =>
                        {
                            var setCommand = $"Set-NetAdapterAdvancedProperty -Name '{adapterName}' -RegistryKeyword '{property}' -RegistryValue '{maxValue}' -ErrorAction SilentlyContinue";
                            await ExecutePowerShellCommandAsync(setCommand);
                ...
                return success;
            }, $"为适配器 {adapterName} 设置最大传输缓存 | Set maximum transmit buffers for adapter {adapterName}", false);
        }

        /// <summary>
        /// 异步关闭网卡节能功能
        /// Asynchronously disable power saving features for network adapter
        /// </summary>
        private async Task<bool> DisablePowerSavingAsync(string adapterName)
        {
            return await _safetyIsolator.ExecuteAsync(async () =>
            {
                var success = false;
                ...
                // 1. 关闭"允许计算机关闭此设备以节约电源"
                // Disable "Allow the computer to turn off this device to save power"
                var disablePowerMgmt = await _safetyIsolator.ExecuteSilentAsync(async () =>
                {
                    var command = $"Get-NetAdapter -Name '{adapterName}' | Get-NetAdapterPowerManagement | Set-NetAdapterPowerManagement -AllowComputerToTurnOffDevice Disabled -ErrorAction SilentlyContinue";
                    await ExecutePowerShellCommandAsync(command);
                });
                ...
                await _safetyIsolator.ExecuteSilentAsync(async () =>
                {
                    ...
                    ";
                    await ExecutePowerShellCommandAsync(wmiCommand);
                });

                return success;
            }, $"为适配器 {adapterName} 关闭节能功能 | Disable power saving for adapter {adapterName}", false);
        }

        /// <summary>
        /// 异步执行PowerShell命令
        /// Asynchronously execute PowerShell command
        /// </summary>
