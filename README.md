...
- Application/ZakYip.Sorting.RuleEngine.Application/Models
  - ParcelProcessingContext.cs：维护包裹处理过程中的上下文数据，包含条码、序号和外部接口响应等信息。
  - ParcelWorkItem.cs：表示待处理的包裹工作项，记录包裹编号、顺序号和任务类型。
- TECHNICAL_DEBT.md：追踪技术债务清单与处理计划，记录时间处理规范违规等未完成事项。
- README.md：项目说明与流程介绍，包含文件结构、更新记录和后续改进方向。
